...
const profileCompletion = computed(() => {
  const fields = [form.fullName, form.nim, form.phone, form.address, form.birthPlace, form.birthDate, form.prodi, form.hobi]
  const filled = fields.filter((f) => f && f.trim && f.trim().length > 0).length
  return Math.round((filled / fields.length) * 100)
})

const docCompletion = computed(() => {
  const fields = [form.sertifikatUrl, form.ktmUrl]
...
<template>
  <DashboardShell title="Edit Profil" subtitle="Perbarui biodata dan kelengkapan berkas kamu" :menu="dashboardMenu">
    ...
    <div v-else class="space-y-6">
      ...
      <section class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="absolute inset-x-0 top-0 h-24 bg-gradient-to-br from-primary/20 via-primary/10 to-violet-500/10" />
        <div class="relative flex flex-col items-center gap-5 p-6 pt-12 sm:flex-row sm:items-center">
          <div class="relative flex-shrink-0">
            ...
            <div
              v-else
              class="flex h-20 w-20 items-center justify-center rounded-2xl border-4 border-white bg-gradient-to-br from-primary to-violet-500 text-2xl font-extrabold text-white shadow-lg"
            >
              {{ (form.fullName || 'U').charAt(0).toUpperCase() }}
            </div>
            <label
              class="absolute -bottom-1 -right-1 flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-primary text-white shadow-md transition hover:bg-primary-dark hover:scale-110"
          ...
          <div class="flex-1 text-center sm:text-left">
            ...
            <div class="mt-2 flex flex-wrap justify-center gap-2 sm:justify-start">
              <span v-if="form.nim" class="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">NIM: {{ form.nim }}</span>
              <span v-if="form.prodi" class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{{ form.prodi }}</span>
            </div>
          </div>
          <div class="flex flex-col items-center gap-1">
            ...
            <span class="text-[10px] font-medium text-slate-400">Kelengkapan</span>
          </div>
              <div class="flex flex-col items-center gap-1">
      ...
      <Transition name="fade-slide">
        <p v-if="message" class="flex items-center gap-2 rounded-xl bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-600">
          <Icon icon="heroicons:check-circle-20-solid" class="text-base" />
          {{ message }}
        </p>
      </Transition>
      <Transition name="fade-slide">
        <p v-if="errorMessage" class="flex items-center gap-2 rounded-xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-600">
          <Icon icon="heroicons:exclamation-circle-20-solid" class="text-base" />
          {{ errorMessage }}
        </p>
      </Transition>
        ...
        <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <form class="grid gap-4 sm:grid-cols-2" @submit.prevent="saveProfile">
            ...
            <div class="flex gap-3 sm:col-span-2">
              <button
                type="button"
                class="flex-1 rounded-xl border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
                @click="router.push('/dashboard-mahasiswa/profil')"
              >
                Batal
              </button>
              <button
                type="submit"
                :disabled="saving"
                class="flex-1 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-primary-dark disabled:cursor-not-allowed disabled:opacity-60"
              >
        ...
        <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div v-if="form.sertifikatUrl" class="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3">
            <div class="flex items-center justify-between">
              ...
              <a :href="resolveFileUrl(form.sertifikatUrl) || '#'" target="_blank" class="text-xs font-semibold text-primary hover:underline">Lihat →</a>
            </div>
          </div>
        ...
        <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div v-if="form.ktmUrl" class="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3">
            <div class="flex items-center justify-between">
              ...
              <a :href="resolveFileUrl(form.ktmUrl) || '#'" target="_blank" class="text-xs font-semibold text-primary hover:underline">Lihat →</a>
            </div>
          </div>
          <label class="flex cursor-pointer flex-col items-center gap-2 rounded-xl border-2 border-dashed border-slate-300 p-6 transition hover:border-primary hover:bg-primary/5">
            <Icon icon="heroicons:cloud-arrow-up-20-solid" class="text-3xl text-slate-400" />
